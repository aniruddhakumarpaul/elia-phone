...
| App standby buckets | System only | am set-standby-bucket | Shell/sysfs | **SMART HUB TIER-1 RUNTIME VERIFIED** via `OptimizationController → SafetyGovernor → SystemActionExecutor → Shizuku UserService` on harmless installed game `com.firsttouchgames.dls7`; baseline `active` restored exactly | Low | Restore exact captured baseline | AVAILABLE — Smart Hub Tier-1 runtime verified |
| AppOps background restriction | System only | cmd appops set RUN_ANY_IN_BACKGROUND | Shell/sysfs | **SMART HUB TIER-1 RUNTIME VERIFIED** via `OptimizationController → SafetyGovernor → SystemActionExecutor → Shizuku UserService` on protected control `com.google.android.youtube`; baseline `allow` restored exactly | Medium | Restore exact captured baseline | AVAILABLE — Smart Hub Tier-1 runtime verified |
