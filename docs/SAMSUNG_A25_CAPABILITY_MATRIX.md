...
| App standby buckets | System only | am set-standby-bucket | Shell/sysfs | Unit-tested; device mutation not run | Low | Restore exact captured baseline | AVAILABLE_PENDING_SAFE_PACKAGE_VALIDATION |
| AppOps background restriction | System only | cmd appops set RUN_ANY_IN_BACKGROUND | Shell/sysfs | Unit-tested; device mutation not run | Medium | Restore exact captured baseline | AVAILABLE_PENDING_SAFE_PACKAGE_VALIDATION |
